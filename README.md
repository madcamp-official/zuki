...
```
trend_signal = 언급 증가율(45점) + 검색 증가율(30점) + 교차검증(25점)
```
...
발굴된 후보 중 신호가 강한 상위 N개는 `POST /api/admin/trends/auto-refresh`가 **트렌드 카드로 자동 생성**하고, 순위에서 밀린 자동 카드는 내린다. 에디터가 직접 만든 카드는 건드리지 않는다.
