...
**후보 키워드는 어디서 오나** — 네이버 데이터랩은 "이 키워드가 얼마나 뜨나?"에만 답하고 "뭐가 뜨나?"는 알려주지 않는다. 키워드를 먼저 줘야 지수를 돌려주는 구조라서, 감시 대상을 만들어내는 발굴 단계(`POST /api/admin/keywords/discover`)를 별도로 둔다:

1. **유튜브 인기 급상승 영상 제목에서 추출** — `videos.list(chart=mostPopular)`는 1 unit밖에 안 든다(검색은 100 unit). 제목에서 "라떼·케이크·크루아상 등으로 끝나는 단어"를 골라낸다.
2. **재료 × 형태 조합 생성** — `흑임자` × `라떼` 같은 조합을 자동 생성. 대부분 검색량 0으로 걸러지고, 살아남는 것이 후보가 된다.
