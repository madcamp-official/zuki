...
**후보 순위는 검색지수가 아니라 트렌드 신호로 매긴다.**

```
trend_signal = 언급 빈도(50점) + 검색 증감률(50점)
```

검색지수를 정렬 기준에 넣으면 에그타르트·밀크티 같은 스테디셀러가 상위를 차지해 발굴이 되지 않는다. 검색지수가 높다는 건 이미 자리잡았다는 뜻이기 때문이다. 우리가 찾는 건 **"최근 글에 자주 나오는데 검색량은 아직 낮은 것"** — 태동기 신호다. 검색지수 40 이상인데 증감률이 5% 미만인 항목은 상시 메뉴로 보고 기본 제외한다.
