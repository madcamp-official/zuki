...
**자주 돌리는 것보다 깊게 파는 게 낫다.** 초기엔 쿼리당 최신 100건만 봤는데, 블로그 글이 쌓이는
속도보다 발굴 주기가 빨라서 매번 같은 글을 다시 읽었다(실측: 후보 144개 중 신규 1개).
지금은 쿼리당 5페이지(500건)까지 훑는다(`NAVER_DISCOVERY_PAGES`, 최대 10 — API의 `start` 상한이 1000).
각 소스에서 더 많은 제목을 보면 "블로그와 카페 양쪽에 다 나오는" 키워드가 잡힐 확률도 함께 오른다.

**지난 유행은 별도 모드로 캔다.** 위 발굴은 전부 최신순(`sort=date`)이라 구조적으로 지금 뜨는 것만
잡힌다 — 흑당버블티는 지금 아무도 글을 안 쓰기 때문이다. 그래서 회고 검색어 32개
(`유행 지난 디저트`, `2022 유행 디저트`, `추억의 음료`, `한물간 디저트` …)를 **정확도순(`sort=sim`)으로
10페이지까지** 훑는 아카이브 모드를 따로 뒀다. 오래전 글에 남아 있는 과거 유행 키워드를 얻고,
그것들은 수집 후 검색량이 낮게 나와 하락기 카드가 되어 "지난 유행" 섹션을 채운다.
`POST /api/admin/pipeline`에 `{"archive": true}`로 실행한다(660회 호출, 한도의 2.6%).
...
`429 errorCode 012 (Rate limit exceeded)`가 난다. `services/naverSearch.ts`가 모든 호출을 하나의 큐로
직렬화해 최소 간격(`NAVER_SEARCH_INTERVAL_MS`, 기본 120ms)을 두고, 그래도 429가 나면 간격을 늘려가며
최대 3회 재시도한다.
