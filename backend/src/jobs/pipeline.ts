 ...
 *
 *   가벼운 발굴  2시간마다  블로그+카페만        → 하루 504회 (한도의 2%)
 *   무거운 발굴  하루 4회   위 + 유튜브          → 하루 2,020unit (한도의 20%)
 *
 * 수집은 데이터랩·유튜브를 크게 쓰므로 하루 1회로 둔다.
 *
 * === 왜 인메모리 락이 필요한가 ===
 *
...
export function schedulePipeline(): void {
  ...
  // 무거운 발굴: 하루 4회, 유튜브 포함. 4 × 505 = 2,020 unit
  const heavyExpr = process.env.DISCOVER_YOUTUBE_CRON || '30 7,12,17,22 * * *';
  cron.schedule(
    heavyExpr,
    () => {
